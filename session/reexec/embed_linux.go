...
func loadEmbeddedReexec(name string, payloadGZ string) (*os.File, error) {
	mf, err := createExecutableMemfd(name, payloadGZ)
	if err != nil {
...
// createExecutableMemfd returns a sealed memfd with the given name, containing the uncompressed gzipped payload
func createExecutableMemfd(name string, payloadGZ string) (_ *os.File, err error) {
	...
	gzreader, err := gzip.NewReader(strings.NewReader(payloadGZ))
	if err != nil {
