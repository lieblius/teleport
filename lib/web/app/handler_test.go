...
func TestDBSCRefresh(t *testing.T) {
	...
	proofJWT, err := makeDBSCProofJWT(deviceKey, challenge)
	require.NoError(t, err)
...
func makeDBSCProofJWT(deviceKey crypto.Signer, challenge string) (string, error) {
	...
	return josejwt.Signed(signer).Claims(josejwt.Claims{
		ID: challenge,
	}).CompactSerialize()
}
